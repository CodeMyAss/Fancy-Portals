...
package com.sniperzciinema.fancyportals.Util;
 ...
 * if (player.isOp())
 * new FancyMessage(ChatColor.YELLOW + "Update for FancyPortals Availble: (" + ChatColor.DARK_AQUA + ChatColor.BOLD + updateName +ChatColor.YELLOW
 * +")").link(updateLink).tooltip("Click here to open the link").send(player);
 * 
 * } */
	...
	private class UpdateRunnable implements Runnable {
		...
		public void run() {
			if (Updater.this.url != null)
				...
				if (read())
					if (versionCheck(Updater.this.versionName))
						if ((Updater.this.versionLink != null) && (Updater.this.type != UpdateType.NO_DOWNLOAD))
						{
							...
							saveFile(new File(Updater.this.plugin.getDataFolder().getParent(),
